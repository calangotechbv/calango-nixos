...
Scope {
    ...
    IpcHandler {
        ...
        function toggle(): void {
            ...
            if (themePanel.visible) {
                ...
                for (var i = 0; i < root.filteredThemes.length; i++) {
                    if (root.filteredThemes[i].originalIndex === root.theme.currentIndex) {
                ...
                if (root.theme.wallpaperMode) {
                    ...
                    themeList.positionViewAtIndex(root.selectedIndex, ListView.Center);
    ...
    PanelWindow {
        ...
        // Click outside the card dismisses. This one only ever sees presses on
        // the screen the card is on: the scrim below covers every screen, but
        // this surface is over it and takes the press first.
        MouseArea {
            anchors.fill: parent
            onClicked: {
                root.theme.previewIndex = -1;
            ...
            ColumnLayout {
                ...
                ListView {
                    ...
                    section.delegate: Item {
                        // Named, and referenced through that name below, because
                        // the unqualified `section` here is the string the view
                        // injects while `themeList.section` is the ListView's
                        // section *configuration* group (a QQuickViewSection).
                        // qmllint's autofix cannot tell them apart and rewrote
                        // this to the latter, which cost a
                        // "Property 'toUpperCase' of object QQuickViewSection is
                        // not a function" every time the theme list drew.
                        id: sectionHeader
