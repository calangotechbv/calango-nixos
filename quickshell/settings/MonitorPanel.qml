    ...
    contentItem: ListView {
      ...
      delegate: Rectangle {
        ...
        Row {
          ...
          Text {
            text: modeDelegate.modelData.split("@")[0]
            color: modeDelegate.modelData === (panel.monitor?.selectedMode ?? "") ? panel.theme.bgBase : panel.theme.textPrimary
          ...
          Text {
            id: rateText
            text: "@" + (modeDelegate.modelData.split("@")[1] ?? "")
            color: modeDelegate.modelData === (panel.monitor?.selectedMode ?? "") ? panel.theme.bgBase : panel.theme.textMuted
            font { pixelSize: 11; family: panel.font }
            anchors.verticalCenter: parent.verticalCenter
          }
        }
        ...
        MouseArea {
          ...
          onClicked: {
            panel.modeSelected(modeDelegate.modelData);
            modeDropdown.close();
          }
        }
      }
    ...
    contentItem: ListView {
      ...
      delegate: Rectangle {
        ...
        Text {
          ...
          text: mirrorDelegate.modelData === "" ? "None" : mirrorDelegate.modelData
          color: mirrorDelegate.modelData === (panel.monitor?.mirrorOf ?? "")
                 ? panel.theme.bgBase
                 : (mirrorDelegate.modelData === "" ? panel.theme.textMuted : panel.theme.textPrimary)
          font { pixelSize: 11; family: panel.font }
        ...
        MouseArea {
          ...
          cursorShape: Qt.PointingHandCursor
          onClicked: {
            panel.mirrorChanged(mirrorDelegate.modelData);
            mirrorDropdown.close();
          }
        }
      }
  ...
  ScrollView {
    ...
    ColumnLayout {
        ...
        Repeater {
          ...
          Rectangle {
            ...
            Text {
              ...
              text: scaleDelegate.modelData + "×"
              color: Math.abs((panel.monitor?.scale ?? -1) - scaleDelegate.modelData) < 0.01 ? panel.theme.bgBase : panel.theme.textPrimary
            ...
            MouseArea {
              anchors.fill: parent
              cursorShape: Qt.PointingHandCursor
              onClicked: panel.scaleSelected(scaleDelegate.modelData)
            }
          }
        }
      }
      ...
      Row {
        ...
        Repeater {
          model: [
            { t: 0, label: "0°",   icon: "monitor" },
            { t: 1, label: "90°",  icon: "rotate-right" },
            { t: 2, label: "180°", icon: "screen-rotation" },
            { t: 3, label: "270°", icon: "rotate-left" },
          ]
          ...
          Rectangle {
            ...
            Row {
              ...
              spacing: 4
              Icon {
                name: transformDelegate.modelData.icon
                anchors.verticalCenter: parent.verticalCenter
                color: (panel.monitor?.transform ?? -1) === transformDelegate.modelData.t ? panel.theme.bgBase : panel.theme.textMuted
                size: 13
              }
              Text {
                text: transformDelegate.modelData.label
                anchors.verticalCenter: parent.verticalCenter
                color: (panel.monitor?.transform ?? -1) === transformDelegate.modelData.t ? panel.theme.bgBase : panel.theme.textPrimary
                font { pixelSize: 10; family: panel.font }
              }
            }
            ...
            MouseArea {
              anchors.fill: parent
              cursorShape: Qt.PointingHandCursor
              onClicked: panel.transformChanged(transformDelegate.modelData.t)
            }
          }
        }
      }
