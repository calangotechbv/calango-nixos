...
QtObject {
    ...
    // Only the actions worth drawing as buttons reach `actions`. "default" is
    // excluded because the freedesktop spec makes it the activation of the
    // notification *itself* rather than a button -- kitty sends Claude Code's
    // notification as exactly one action, {identifier: "default", text: " "},
    // and that single space is what drew the blank 24px pill. Any other action
    // with a blank label is dropped for the same reason: a button with nothing
    // written on it cannot be acted on, and it has no accessible name either.
    function _readActions(): void {
        if (!notification) return;
        hasDefaultAction = notification.actions.some(function(a) {
            return a.identifier === "default";
        });
        actions = notification.actions.filter(function(a) {
            return a.identifier !== "default" && (a.text || "").trim() !== "";
        }).map(function(a) {
            return { identifier: a.identifier, text: a.text };
        });
    }

    // Clicking the card body is the spec's activation gesture: invoke "default"
    // when the sender offered one (kitty's focuses the terminal window), and
    // otherwise fall back to the dismiss this always did.
    function activate(): void {
        if (hasDefaultAction) invokeAction("default");
        else dismiss();
    }
